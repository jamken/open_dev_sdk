...
OVS RECORD服务的主要目标是向OVD提供录像分片（HTTP）直接上传功能，中间无需经由流媒体服务器进行录制，达到降低服务器成本，减少中间环节（瓶颈点），提高录像稳定性。
...
在正常流程中，每个分片上传需要调用直存服务接口两次（一次create,一次save），开销比较大。考虑到录像分片一般是连续上传的情况，在直存服务的create请求中，
...
RECORD服务对外提供HTTP类型api，设备只需要支持http协议，即可调用直存服务进行切片上传。每个设备在OVC上对应一个录像直存通道，每个直存通道拥有一个独立的HTTP URL。
